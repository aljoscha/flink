 ...
 *
 * Linear Regression with BGD(batch gradient descent) algorithm is an iterative algorithm and works
 * as follows:
 *
 ...
 * iterations (as in this implementation). With enough iteration, the algorithm can minimize the
 * cost function and find the best parameters This is the Wikipedia entry for the
 * [[http://en.wikipedia.org/wiki/Linear_regressionLinearregression]] and
 * [[http://en.wikipedia.org/wiki/Gradient_descentGradientdescentalgorithm]].
 *
 * This implementation works on one-dimensional data and finds the best two-dimensional theta to fit
 * the target.
 *
 ...
 *     are separated by newline characters. For example `"-0.02 -0.04\n5.3 10.6\n"`gives two data
 *     points (x=-0.02, y=-0.04) and (x=5.3, y=10.6).
 *
