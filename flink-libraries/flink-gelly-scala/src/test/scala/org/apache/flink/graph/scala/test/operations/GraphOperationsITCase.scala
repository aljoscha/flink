...
class GraphOperationsITCase(mode: MultipleProgramsTestBase.TestExecutionMode)
  ...
  def testSubGraphSugar() {
    ...
    val graph: Graph[Long, Long, Long] = Graph.fromDataSet(
      ...
      env)
    val res = graph
      .subgraph(vertex => vertex.getValue > 2, edge => edge.getValue > 34)
      .getEdges
      .collect()
      .toList
  ...
  def testUnion() {
    ...
    val vertices: List[Vertex[Long, Long]] =
      List[Vertex[Long, Long]](new Vertex[Long, Long](6L, 6L))
    val edges: List[Edge[Long, Long]] = List[Edge[Long, Long]](new Edge[Long, Long](6L, 1L, 61L))
  ...
  def testDifferenceNoCommonVertices() {
    ...
    val vertices: List[Vertex[Long, Long]] =
      List[Vertex[Long, Long]](new Vertex[Long, Long](6L, 6L))
    val edges: List[Edge[Long, Long]] = List[Edge[Long, Long]](new Edge[Long, Long](6L, 6L, 66L))
