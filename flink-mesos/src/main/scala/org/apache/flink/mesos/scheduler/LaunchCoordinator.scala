...
class LaunchCoordinator(
    ...
    optimizerBuilder
      .withLeaseRejectAction(new Action1[VirtualMachineLease]() {
        def call(lease: VirtualMachineLease) {
          LOG.info(
            s"Declined offer ${lease.getId} from ${lease.hostname()} "
   ...
   */
  when(Suspended) { case Event(msg: Connected, data: GatherData) =>
   ...
   */
  onTransition { case _ -> Idle =>
    assert(nextStateData.tasks.isEmpty)
  }
  ...
  when(GatheringOffers, stateTimeout = GATHER_DURATION) {
    ...
    case Event(offers: ResourceOffers, data: GatherData) =>
      ...
      if (LOG.isInfoEnabled) {
        ...
        LOG.info(
          s"Received offer(s) of $mem MB, $cpus cpus, $gpus gpus, " +
    ...
    case Event(StateTimeout, data: GatherData) =>
      ...
      LOG.info(
        s"Processing ${remaining.size} task(s) against ${data.newLeases.length}"
          + s" new offer(s) plus outstanding offers.")

      // attempt to assign the outstanding tasks using the optimizer
      val result =
        optimizer.scheduleOnce(data.tasks.map(_.taskRequest).asJava, data.newLeases.asJava)
      ...
      if (LOG.isInfoEnabled) {
        ...
        for (vm <- optimizer.getVmCurrentStates.asScala) {
          ...
          LOG.info(
            s"  ${vm.getHostname} has ${lease.memoryMB()} MB," +
      ...
      for ((hostname, assignments) <- result.getResultMap.asScala) {
        ...
        if (LOG.isInfoEnabled) {
          LOG.info(
            s"Launched ${launchedTasks.length} task(s) on ${hostname}"
  ...
  whenUnhandled {
    ...
    case Event(offer: OfferRescinded, data: GatherData) =>
      ...
      stay using data.copy(newLeases =
        data.newLeases.filterNot(_.getOffer.getId == offer.offerId()))
...
object LaunchCoordinator {
  ...
  private def processAssignments(
    ...
    val launches = Protos.Offer.Operation
      .newBuilder()
      .setType(Protos.Offer.Operation.Type.LAUNCH)
      .setLaunch(
        Protos.Offer.Operation.Launch
          .newBuilder()
          .addAllTaskInfos(assignments.getTasksAssigned.asScala.map(taskInfo).asJava))
      .build()
