 ...
 */
class ReconciliationCoordinator(flinkConfig: Configuration, schedulerDriver: SchedulerDriver)
    extends Actor
    with FSM[TaskState, ReconciliationData] {
  ...
  when(Suspended) {
    case Event(reconcile: Reconcile, data: ReconciliationData) =>
      ...
      stay using data.copy(remaining =
        if (reconcile.replace) tasks.toMap else data.remaining ++ tasks)
  ...
  when(Idle) { case Event(reconcile: Reconcile, _) =>
