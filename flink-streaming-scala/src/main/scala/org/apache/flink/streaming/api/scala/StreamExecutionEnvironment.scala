...
class StreamExecutionEnvironment(javaEnv: JavaEnv) {
   ...
   * [[org.apache.flink.streaming.api.datastream.KeyedStreamkeyedstreams]], as well as state
   * maintained directly by the user code that implements
   * [[org.apache.flink.streaming.api.checkpoint.CheckpointedFunctionCheckpointedFunction]].
   *
   * The [[org.apache.flink.runtime.state.memory.MemoryStateBackend]], for example, maintains the
   * state in heap memory, as objects. It is lightweight without extra dependencies, but can
   * checkpoint only small states (some counters).
   *
   * In contrast, the [[org.apache.flink.runtime.state.filesystem.FsStateBackend]] stores
   ...
   * [[org.apache.flink.core.fs.FileInputSplitFileInputSplits]] to be processed, forwards them to
   * the downstream [[ContinuousFileReaderOperatorreaders]] to read the actual data, and exits,
   ...
   * call clears previously registered [[org.apache.flink.api.dag.Transformationtransformations]].
   *
   * @return
   ...
   * [[org.apache.flink.api.dag.Transformationtransformations]]. Clearing the transformations
