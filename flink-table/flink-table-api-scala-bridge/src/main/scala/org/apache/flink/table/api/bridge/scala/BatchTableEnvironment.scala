...
/**
 * The [[TableEnvironment]] for a Scala batch [[ExecutionEnvironment]] that works with [[DataSet]]
 * s.
 *
...
trait BatchTableEnvironment extends TableEnvironment {
   ...
   * functions can be referenced in Table API and SQL queries.
   *
   * @param name
   ...
   * }}}
   *
   * 2. Reference input fields by position: In this mode, fields are simply renamed. This mode can
   * only be used if the input type has a defined field order (tuple, case class, Row) and none of
   ...
   *   The [[DataSet]] to be converted.
   * @param fields
   *   The fields expressions to map original fields of the DataSet to the fields of the [[Table]].
   * @tparam T
   *   The type of the [[DataSet]].
   * @return
   *   The converted [[Table]].
  ...
  /**
   * Creates a view from the given [[DataSet]]. Registered views can be referenced in SQL queries.
   *
   * The field names of the [[Table]] are automatically derived from the type of the [[DataSet]].
   *
   ...
   *
   * @param name
   *   The name under which the [[DataSet]] is registered in the catalog.
   * @param dataSet
   *   The [[DataSet]] to register.
   * @tparam T
   *   The type of the [[DataSet]] to register.
   * @deprecated
  ...
  /**
   * Creates a view from the given [[DataSet]] in a given path. Registered tables can be referenced
   * in SQL queries.
   *
   * The field names of the [[Table]] are automatically derived from the type of the [[DataSet]].
   *
   ...
   *
   * @param path
   *   The path under which the [[DataSet]] is created. See also the [[TableEnvironment]] class
   *   description for the format of the path.
   * @param dataSet
   *   The [[DataSet]] out of which to create the view.
   * @tparam T
   *   The type of the [[DataSet]].
   */
   ...
   *   The [[DataSet]] to register.
   * @param fields
   *   The fields expressions to map original fields of the DataSet to the fields of the View.
   * @tparam T
   *   The type of the [[DataSet]] to register.
   * @deprecated
   *   use [[createTemporaryView]]
   ...
   *
   * It calls the ExecutionEnvironment#execute on the underlying [[ExecutionEnvironment]]. In
   * contrast to the [[TableEnvironment]] this environment translates queries eagerly.
   *
   * @param jobName
   ...
   *
   * The following example shows how to read from a connector using a JSON format and registering a
   * temporary table as "MyTable":
   *
   * {{{
   *
   * tableEnv
   ...
   * @deprecated
   *   The SQL `CREATE TABLE` DDL is richer than this part of the API. This method might be
...
object BatchTableEnvironment {
  ...
  def create(
    ...
    try {
      ...
      con
        .newInstance(executionEnvironment, tableConfig, catalogManager, moduleManager)
