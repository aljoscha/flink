      ...
      case r: RichSqlInsert if r.getStaticPartitions.nonEmpty =>
        r.getSource match {
          ...
          case values: SqlCall if values.getKind == SqlKind.VALUES =>
            val newSource =
              appendPartitionProjects(r, validator, typeFactory, values, r.getStaticPartitions)
...
object PreValidateReWriter {
  ...
  def appendPartitionProjects(
    ...
    val targetRowType =
      createTargetRowType(typeFactory, calciteCatalogReader, table, sqlInsert.getTargetColumnList)
  ...
  private def rewriteSelect(
    ...
    0 until targetRowType.getFieldList.length foreach { idx =>
  ...
  private def rewriteValues(
      ...
      assignedFields: util.LinkedHashMap[Integer, SqlNode]): SqlCall = {
    val fixedNodes = new util.ArrayList[SqlNode]
    0 until values.getOperandList.size() foreach { valueIdx =>
      ...
      0 until targetRowType.getFieldList.length foreach { fieldIdx =>
  ...
  private def createTargetRowType(
    ...
    for (node <- targetColumnList) {
      ...
      val targetField =
        SqlValidatorUtil.getTargetField(rowType, typeFactory, id, catalogReader, relOptTable)
