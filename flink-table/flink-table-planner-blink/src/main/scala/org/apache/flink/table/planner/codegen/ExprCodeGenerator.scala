...
class ExprCodeGenerator(ctx: CodeGeneratorContext, nullableInput: Boolean)
  ...
  def generateConverterResultExpression(
    ...
    val input1AccessExprs = input1Mapping.map {
      ...
      case TimeIndicatorTypeInfo.PROCTIME_STREAM_MARKER =>
        ...
        generateNullLiteral(new TimestampType(true, TimestampKind.PROCTIME, 3), ctx.nullCheck)
    ...
    val input2AccessExprs = input2Type match {
      case Some(ti) =>
        input2Mapping
          .map(idx =>
            generateInputAccess(ctx, ti, input2Term.get, idx, nullableInput, ctx.nullCheck))
          .toSeq
  ...
  def generateResultExpression(
      ...
      allowSplit: Boolean,
      methodName: String): GeneratedExpression = {
    // initial type check
