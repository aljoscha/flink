...
object FunctionGenerator {
  ...
  val INTEGRAL_TYPES = Array(INTEGER, SMALLINT, INTEGER, BIGINT)
  ...
  addSqlFunction(FLOOR, Seq(DOUBLE), new FloorCeilCallGen(BuiltInMethod.FLOOR.method))

  addSqlFunction(FLOOR, Seq(DECIMAL), new FloorCeilCallGen(BuiltInMethods.FLOOR_DEC))

  addSqlFunction(CEIL, Seq(DOUBLE), new FloorCeilCallGen(BuiltInMethod.CEIL.method))

  addSqlFunction(CEIL, Seq(DECIMAL), new FloorCeilCallGen(BuiltInMethods.CEIL_DEC))
  ...
  addSqlFunction(
    TIMESTAMP_DIFF,
    Seq(RAW, TIMESTAMP_WITHOUT_TIME_ZONE, TIMESTAMP_WITHOUT_TIME_ZONE),
  ...
  addSqlFunction(TIMESTAMP_DIFF, Seq(RAW, DATE, DATE), new TimestampDiffCallGen)
  ...
  addSqlFunction(
    CEIL,
    Seq(TIMESTAMP_WITH_LOCAL_TIME_ZONE, RAW),
    new FloorCeilCallGen(BuiltInMethod.CEIL.method, Some(BuiltInMethods.TIMESTAMP_CEIL_TIME_ZONE)))

  addSqlFunction(CURRENT_DATE, Seq(), new CurrentTimePointCallGen(false))

  addSqlFunction(CURRENT_TIME, Seq(), new CurrentTimePointCallGen(false))

  addSqlFunction(NOW, Seq(), new CurrentTimePointCallGen(false))

  addSqlFunction(CURRENT_TIMESTAMP, Seq(), new CurrentTimePointCallGen(false))

  addSqlFunction(LOCALTIME, Seq(), new CurrentTimePointCallGen(true))

  addSqlFunction(LOCALTIMESTAMP, Seq(), new CurrentTimePointCallGen(true))
  ...
  INTEGRAL_TYPES foreach (dt =>
    addSqlFunctionMethod(TO_TIMESTAMP, Seq(dt), BuiltInMethods.LONG_TO_TIMESTAMP))

  FRACTIONAL_TYPES foreach (dt =>
    addSqlFunctionMethod(TO_TIMESTAMP, Seq(dt), BuiltInMethods.DOUBLE_TO_TIMESTAMP))

  addSqlFunctionMethod(TO_TIMESTAMP, Seq(DECIMAL), BuiltInMethods.DECIMAL_TO_TIMESTAMP)

  INTEGRAL_TYPES foreach (dt =>
    addSqlFunctionMethod(FROM_UNIXTIME, Seq(dt), BuiltInMethods.FROM_UNIXTIME))
  ...
  def getCallGenerator(
      ...
      resultType: LogicalType): Option[CallGenerator] = sqlOperator match {
