...
class FlinkRelMdDistinctRowCount private extends MetadataHandler[BuiltInMetadata.DistinctRowCount] {
  ...
  def getDistinctRowCount(
    ...
    if (isUnique) {
      ...
      val distinctCount = groupKey.asList().foldLeft(1d) { (ndv, g) =>
  ...
  private def getDistinctRowCountOfAggregate(
    ...
    val ndvOfColsInAggCalls = aggCalls.foldLeft(1d) { (ndv, aggCall) =>
  ...
  def getDistinctRowCount(
    ...
    val rexBuilder = rel.getCluster.getRexBuilder
    val distinctRowCounts: Seq[JDouble] = rel.getInputs map { input =>
  ...
  def getDistinctRowCount(
    ...
    subset.getRels.foldLeft(null.asInstanceOf[JDouble]) { (min, rel) =>
      try {
