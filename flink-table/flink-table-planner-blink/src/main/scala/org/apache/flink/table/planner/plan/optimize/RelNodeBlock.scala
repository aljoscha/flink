 ...
 * | | Filter(a1>=70) Filter(a1<70) \ / Join(a1=b2) / \ Project(a1,b1) Project(b2,c2)
 * | | Filter(a>0) Filter(c is not null) \ / Project(a,b,c)
 * | TableScan }}}
 *
...
class RelNodeBlockPlanBuilder private (config: TableConfig) {
  ...
  private def buildBlock(
    ...
    if (validBreakPoint && (createNewBlockWhenMeetValidBreakPoint || hasDiffBlockOutputNodes)) {
      ...
      node.getInputs.foreach { child =>
        buildBlock(child, childBlock, createNewBlockWhenMeetValidBreakPoint = false)
      }
    } else {
      ...
      node.getInputs.foreach { child =>
        buildBlock(child, currentBlock, newCreateNewBlockWhenMeetValidBreakPoint)
      }
    }
  }
...
object RelNodeBlockPlanBuilder {
   ...
   *   Sink-RelNodeBlocks, each Sink-RelNodeBlock is a tree.
   */
  def buildRelNodeBlockPlan(sinkNodes: Seq[RelNode], config: TableConfig): Seq[RelNodeBlock] = {
