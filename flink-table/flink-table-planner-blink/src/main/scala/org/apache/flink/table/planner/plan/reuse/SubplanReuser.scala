...
import org.apache.flink.table.planner.plan.nodes.physical.common.{
  CommonPhysicalLegacyTableSourceScan,
...
object SubplanReuser {
  ...
  class SubplanReuseShuttle(context: SubplanReuseContext) extends DefaultRelShuttle {
    ...
    private def visitInputs(rel: RelNode): RelNode = {
      rel.getInputs.zipWithIndex.foreach { case (input, index) =>
