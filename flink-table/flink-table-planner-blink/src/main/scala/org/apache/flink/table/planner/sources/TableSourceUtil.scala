...
object TableSourceUtil {
  ...
  def getSourceRowTypeFromSource(
    ...
    if (streaming) {
      ...
      proctimeAttributes.foreach { proctimeField =>
        ...
        val proctimeType = new TimestampType(true, TimestampKind.PROCTIME, 3)
  ...
  def getRowtimeExtractionExpression(
    ...
    val expr =
      constructExpression(extractor, accessedFields).accept(new ExpressionConverter(relBuilder))
    relBuilder.clear()
    expr
  }
