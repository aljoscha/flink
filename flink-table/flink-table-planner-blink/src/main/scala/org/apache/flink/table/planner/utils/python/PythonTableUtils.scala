...
object PythonTableUtils {
   ...
   *
   * @param data
   *   The unpickled python data.
   * @param dataType
   *   The python data type.
   * @param config
   *   The execution config used to create serializer.
   * @return
  ...
  def getInputFormat(
    ...
    new CollectionInputFormat(
      data.map(converter(_).asInstanceOf[Row]),
  ...
  def getCollectionInputFormat[T](
    ...
    new CollectionInputFormat[T](
      data.map(converter(_).asInstanceOf[T]),
      dataType.createSerializer(config))
  }

  /**
   * Creates a converter that converts `obj` to the type specified by the data type, or returns null
   * if the type of obj is unexpected because Python doesn't enforce the type.
   */
  private def convertTo(dataType: TypeInformation[_]): Any => Any = dataType match {
    case _ if dataType == Types.BOOLEAN =>
      (obj: Any) =>
        nullSafeConvert(obj) { case b: Boolean =>
          b
        }

    case _ if dataType == Types.BYTE =>
      (obj: Any) =>
        nullSafeConvert(obj) {
          case c: Byte  => c
    ...
    case _ if dataType == Types.SHORT =>
      (obj: Any) =>
        nullSafeConvert(obj) {
    ...
    case _ if dataType == Types.FLOAT =>
      (obj: Any) =>
        nullSafeConvert(obj) {
          case c: Float  => c
    ...
    case _ if dataType == Types.DOUBLE =>
      (obj: Any) =>
        nullSafeConvert(obj) {
    ...
    case _ if dataType == Types.DECIMAL =>
      (obj: Any) =>
        nullSafeConvert(obj) { case c: java.math.BigDecimal =>
          c
        }

    case _ if dataType == Types.SQL_DATE =>
      (obj: Any) =>
        nullSafeConvert(obj) { case c: Int =>
          val millisLocal = c.toLong * 86400000
          val millisUtc = millisLocal - getOffsetFromLocalMillis(millisLocal)
          new Date(millisUtc)
        }

    case _ if dataType == Types.SQL_TIME =>
      (obj: Any) =>
        nullSafeConvert(obj) {
    ...
    case _ if dataType == Types.SQL_TIMESTAMP =>
      (obj: Any) =>
        nullSafeConvert(obj) {
    ...
    case _ if dataType == Types.INTERVAL_MILLIS() =>
      (obj: Any) =>
        nullSafeConvert(obj) {
          case c: Long => c / 1000
    ...
    case _ if dataType == Types.STRING =>
      (obj: Any) =>
        nullSafeConvert(obj) { case _ =>
          obj.toString
        }
    ...
    case _: PrimitiveArrayTypeInfo[_] | _: BasicArrayTypeInfo[_, _] |
      ...
      (obj: Any) =>
        nullSafeConvert(obj) {
          case c: java.util.List[_] =>
            createArray(elementType, c.size(), i => elementFromJava(c.get(i)), boxed)
    ...
    case m: MapTypeInfo[_, _] =>
      ...
      (obj: Any) =>
        nullSafeConvert(obj) { case javaMap: java.util.Map[_, _] =>
  ...
  def getOffsetFromLocalMillis(millisLocal: Long): Int = {
    ...
    if (offset != result) {
      ...
      if (result != offset) {
        ...
        val localTime = LocalTime.ofNanoOfDay(Math.floorMod(millisLocal, 86400000) * 1000 * 1000)
 ...
 *
 * @param inputFormat
 *   The input format which contains the python data collection, usually created by
 *   PythonTableUtils#getInputFormat method
 * @param rowTypeInfo
