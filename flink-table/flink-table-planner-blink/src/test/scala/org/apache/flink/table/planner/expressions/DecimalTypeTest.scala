...
//    // from String
    ...
    // from double
    testAllApis('f3.cast(DataTypes.DECIMAL(38, 0)), "CAST(f3 AS DECIMAL)", "4")
  }
  ...
  def testDecimalCasting(): Unit = {
    ...
    // to double
    testAllApis('f0.cast(DataTypes.DOUBLE), "CAST(f0 AS DOUBLE)", "1.2345678912345679E8")

    // to int
    testAllApis('f4.cast(DataTypes.INT), "CAST(f4 AS INT)", "123456789")

    // to long
    testAllApis('f4.cast(DataTypes.BIGINT()), "CAST(f4 AS BIGINT)", "123456789")

    // to boolean (not SQL compliant)
    testTableApi('f1.cast(DataTypes.BOOLEAN), "true")

    testTableApi('f5.cast(DataTypes.BOOLEAN), "false")
  ...
  def testDecimalArithmetic(): Unit = {
    ...
    testAllApis('f1 + BigDecimal("12.3"), "f1 + 12.3", "123456789123456789123456801.3")
  ...
  def testPlusMinus(): Unit = {
    ...
    testAllApis('f15 + 'f16, "f15 + f16", "300.0246913578012345678901234567")

    testAllApis('f15 - 'f16, "f15 - f16", "-100.0000000000012345678901234567")

    // 10 digits integral part
    testAllApis('f17 + 'f18, "f17 + f18", "null")
  ...
  def testMultiply(): Unit = {
    ...
    testAllApis('f24 * 'f24, "f24 * f24", "1.000000000000")

    testAllApis('f24 * 'f25, "f24 * f25", "2.0000000000000000")

    testAllApis('f26 * 'f26, "f26 * f26", "0.00010000000000000000000000000000000000")
    ...
    testAllApis('f27 * 'f28, "f27 * f28", "0.00000060000000000000")

    // result overflow
    testAllApis('f29 * 'f29, "f29 * f29", "null")

    //(60,40)=>(38,38), no space for integral part
    testAllApis('f30 * 'f30, "f30 * f30", "null")
  }
  ...
  def testExactionFunctions(): Unit = {
    ...
    testAllApis('f48.abs(), "abs(f48)", "3.14")

    testAllApis((-'f48).abs(), "abs(-f48)", "3.14")

    testAllApis('f48.floor(), "floor(f48)", "3")

    testAllApis('f48.ceil(), "ceil(f48)", "4")

    // calcite: SIGN(Decimal(p,s))=>Decimal(p,s)
    testAllApis('f48.sign(), "sign(f48)", "1.00")

    testAllApis((-'f48).sign(), "sign(-f48)", "-1.00")
    testAllApis(('f48 - 'f48).sign(), "sign(f48 - f48)", "0.00")
