  ...
  def testDeriveNullFilterAfterJoinReorder(): Unit = {
    ...
    util.addTableSource(
      ...
      FlinkStatistic
        .builder()
        .tableStats(
          new TableStats(500000L, Map("a6" -> builderA.build(), "b6" -> builderB.build())))
        .build())
    ...
    util.addTableSource(
      ...
      FlinkStatistic
        .builder()
        .tableStats(
          new TableStats(500000L, Map("a7" -> builderA.build(), "b7" -> builderB.build())))
        .build())
    ...
    util.addTableSource(
      ...
      FlinkStatistic
        .builder()
        .tableStats(
          new TableStats(500000L, Map("a8" -> builderA.build(), "b8" -> builderB.build())))
        .build())

    util.getTableEnv.getConfig.getConfiguration
      .setLong(JoinDeriveNullFilterRule.TABLE_OPTIMIZER_JOIN_NULL_FILTER_THRESHOLD, 10000L)
    val sql =
      s"""
         |SELECT * FROM T6
