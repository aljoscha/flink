...
class FlinkRelMdRowCountTest extends FlinkRelMdHandlerTestBase {
  ...
  def testGetRowCountOnExpand(): Unit = {
    Array(logicalExpand, flinkLogicalExpand, batchExpand, streamExpand).foreach { expand =>
      assertEquals(150.0, mq.getRowCount(expand))
    }
  }
  ...
  def testGetRowCountOnExchange(): Unit = {
    Array(batchExchange, streamExchange).foreach { exchange =>
      assertEquals(50.0, mq.getRowCount(exchange))
    }
  }
  ...
  def testGetRowCountOnRank(): Unit = {
    Array(logicalRank, flinkLogicalRank, batchLocalRank, streamRank).foreach { rank =>
      assertEquals(5.0, mq.getRowCount(rank))
    }
    // TODO FLINK-12282
    assertEquals(1.0, mq.getRowCount(batchGlobalRank))

    Array(logicalRank2, flinkLogicalRank2, batchLocalRank2, streamRank2).foreach { rank =>
    ...
    Array(logicalRowNumber, flinkLogicalRowNumber, streamRowNumber).foreach { rank =>
      assertEquals(4.0, mq.getRowCount(rank))
    }

    Array(
      logicalRankWithVariableRange,
      flinkLogicalRankWithVariableRange,
      streamRankWithVariableRange).foreach { rank =>
      assertEquals(5.0, mq.getRowCount(rank))
    }
  }
  ...
  def testGetRowCountOnAggregate(): Unit = {
    Array(
      ...
      assertEquals(7.0, mq.getRowCount(agg))
    }
