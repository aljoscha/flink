...
class FlinkRelMdSelectivityTest extends FlinkRelMdHandlerTestBase {
  ...
  def testGetSelectivityOnExpand(): Unit = {
    ...
    val expandProjects = ExpandUtil.createExpandProjects(
      ...
      Array.empty[Integer])
    val expand =
      new FlinkLogicalExpand(ts.getCluster, ts.getTraitSet, ts, expandOutputType, expandProjects, 2)
  ...
  def testGetSelectivityOnAggregate(): Unit = {
    ...
    val expandProjects = ExpandUtil.createExpandProjects(
      ...
      Array.empty[Integer])
    val expand =
      new LogicalExpand(ts.getCluster, ts.getTraitSet, ts, expandOutputType, expandProjects, 4)
