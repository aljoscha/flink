...
class CalcITCase extends BatchTestBase {
  ...
  def testManySelectWithFilter(): Unit = {
    val data = Seq((true, 1, 2, 3, 4, 5, 6, 7), (false, 1, 2, 3, 4, 5, 6, 7))
    ...
    checkResult(
      ...
      """.stripMargin,
      Seq(
        row(true, 1, 2, 3, 4, 5, 6, 7, true, 1, 2, 6, 3, 4, 5, 7, 7, 6, 5, 4, 3, 2, 1, true, 7, 5,
          4, 3, 6, 2, 1, true, 2, true, 1, 6, 5, 4, 7, 3, true, 1, 2, 3, 4, 5, 6, 7, true, 1, 2, 6,
          3, 4, 5, 7, 7, 6, 5, 4, 3, 2, 1, true, 7, 5, 4, 3, 6, 2, 1, true, 2, true, 1, 6, 5, 4, 7,
          3)))
  }
  ...
  def testFilterOnString(): Unit = {
    checkResult(
      ...
      Seq(row(3, 2L, "Hello world"), row(4, 3L, "Hello world, how are you?")))
  }
  ...
  def testDisjunctivePredicate(): Unit = {
    checkResult(
      ...
      Seq(row(1, 1L, "Hi"), row(21, 6L, "Comment#15")))
  }
  ...
  def testTimestampSemantics(): Unit = {
    ...
    val data = Seq(row(ldt, ldt.toInstant(new_york.getRules.getOffset(ldt))))
  ...
  def testUserDefinedScalarFunctionWithParameter(): Unit = {
    ...
    checkResult("SELECT c FROM SmallTable3 where RichFunc2(c)='ABC#Hello'", Seq(row("Hello")))
  }
  ...
  def testUserDefinedScalarFunctionWithDistributedCache(): Unit = {
    ...
    checkResult("SELECT c FROM SmallTable3 where RichFunc3(c)=true", Seq(row("Hello")))
  }
  ...
  def testPojoField(): Unit = {
    ...
    checkResult("SELECT a FROM MyTable", Seq(row(row(5, 105)), row(row(6, 11)), row(row(7, 12))))
  }
...
//  }
  ...
  def testRowType(): Unit = {
    ...
    checkResult(
      ...
      Seq(row(row(1, "Hi", true)), row(row(1, "Hi", true)), row(row(1, "Hi", true))))

    // primitive type
    checkResult(
      "SELECT ROW(1, a, b) FROM SmallTable3",
      Seq(row(row(1, 1, 1L)), row(row(1, 2, 2L)), row(row(1, 3, 2L))))
  }
  ...
  def testRowTypeWithDecimal(): Unit = {
    ...
    checkResult(
      ...
      Seq(row(row(d, 1, "Hi")), row(row(d, 2, "Hello")), row(row(d, 3, "Hello world"))))
  }
  ...
  def testArrayType(): Unit = {
    ...
    checkResult(
      ...
      Seq(row("[1, 30, 10, 1]"), row("[2, 30, 10, 2]"), row("[2, 30, 10, 3]")))

    // non-primitive type
    checkResult(
      "SELECT ARRAY['Test', c] FROM SmallTable3",
      Seq(row("[Test, Hi]"), row("[Test, Hello]"), row("[Test, Hello world]")))
  }
  ...
  def testMapType(): Unit = {
    ...
    checkResult(
      ...
      Seq(row("{1=Hello, 2=Hi}"), row("{1=Hello, 2=Hi}"), row("{1=Hello, 2=Hi}")))
  ...
  def testSelectStarFromNestedTable(): Unit = {
    ...
    val table =
      BatchTableEnvUtil.fromCollection(tEnv, Seq(((0, 0), "0"), ((1, 1), "1"), ((2, 2), "2")))
    tEnv.registerTable("MyTable", table)

    checkResult(
      "SELECT * FROM MyTable",
      Seq(row(row(0, 0), "0"), row(row(1, 1), "1"), row(row(2, 2), "2")))
  }

  @Test
  def testSelectStarFromNestedValues(): Unit = {
    val table = BatchTableEnvUtil.fromCollection(tEnv, Seq((0L, "0"), (1L, "1"), (2L, "2")), "a, b")
    tEnv.registerTable("MyTable", table)
  ...
  def testCurrentTimestamp(): Unit = {
    ...
    val result = executeQuery(table)
    val ts1 = TimestampData
      .fromLocalDateTime(result.toList.head.getField(0).asInstanceOf[LocalDateTime])
      .getMillisecond
  ...
  def testDateCompareWithDateString(): Unit = {
    checkResult(
      ...
      Seq(row(localDate("2017-12-12")), row(localDate("2017-12-12"))))
  }
  ...
  def testCalcBinary(): Unit = {
    registerCollection(
      "BinaryT",
      nullData3.map((r) =>
        row(r.getField(0), r.getField(1), r.getField(2).toString.getBytes(StandardCharsets.UTF_8))),
  ...
  def testOrderByBinary(): Unit = {
    ...
    conf.getConfiguration.setInteger(
      ...
      1)
    conf.getConfiguration.setBoolean(BatchPhysicalSortRule.TABLE_EXEC_RANGE_SORT_ENABLED, true)
    checkResult(
  ...
  def testGroupByBinary(): Unit = {
    registerCollection(
      "BinaryT2",
      nullData3.map((r) =>
        row(r.getField(0), r.getField(1).toString.getBytes(StandardCharsets.UTF_8), r.getField(2))),
