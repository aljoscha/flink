...
abstract class DistinctAggregateITCaseBase extends BatchTestBase {
  ...
  def testSingleDistinctAgg(): Unit = {
    checkResult("SELECT COUNT(DISTINCT a) FROM Table3", Seq(row(21)))
    checkResult("SELECT COUNT(DISTINCT a) FROM EmptyTable3", Seq(row(0)))
  }
  ...
  def testSingleDistinctAggAndOneOrMultiNonDistinctAgg(): Unit = {
    checkResult("SELECT COUNT(DISTINCT c), SUM(a) FROM NullTable3", Seq(row(18, 231)))

    checkResult("SELECT COUNT(DISTINCT b), COUNT(c) FROM NullTable3", Seq(row(6, 18)))
  }

  @Test
  def testMultiDistinctAggOnDifferentColumn(): Unit = {
    checkResult("SELECT COUNT(DISTINCT a), SUM(DISTINCT b) FROM Table3", Seq(row(21, 21)))
  ...
  def testSingleDistinctAggWithGroupBy(): Unit = {
    ...
    checkResult("SELECT a, COUNT(a), SUM(DISTINCT b) FROM EmptyTable3 GROUP BY a", Seq())
  }
  ...
  def testSingleDistinctAggWithGroupByAndCountStar(): Unit = {
    ...
    checkResult("SELECT a, COUNT(*), SUM(DISTINCT b) FROM EmptyTable3 GROUP BY a", Seq())
  }
