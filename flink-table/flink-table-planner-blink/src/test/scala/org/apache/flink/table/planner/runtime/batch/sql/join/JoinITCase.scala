...
class JoinITCase(expectedJoinType: JoinType) extends BatchTestBase {
  ...
  def testInnerJoinWithNonEquiJoinPredicate(): Unit = {
    checkResult(
      ...
      Seq(row("Hello world, how are you?", "Hallo Welt wie"), row("I am fine.", "Hallo Welt wie")))
  }
  ...
  def testSingleRowFullOuterJoinWithoutEqualCond(): Unit = {
    if (expectedJoinType == NestedLoopJoin) {
      checkResult(
        ...
        Seq(row("Hello world", "Hi"), row("Hello", "Hi"), row("Hi", null)))
    }
  }
  ...
  def testSingleRowFullOuterJoinWithoutEqualCondNoMatch(): Unit = {
    if (expectedJoinType == NestedLoopJoin) {
      checkResult(
        ...
        Seq(row("Hello world", null), row("Hello", null), row("Hi", null), row(null, "Hi")))
    }
  }
  ...
  def testSingleRowJoin(): Unit = {
    if (expectedJoinType == NestedLoopJoin) {
      checkResult(
        ...
        Seq(row(5L, 1, 1L, "Hi"), row(5L, 2, 2L, "Hello"), row(5L, 3, 2L, "Hello world")))
  ...
  def testJoinCollation(): Unit = {
    checkResult(
      ...
      """.stripMargin,
      Seq(row(1, 1, 2L, 1L), row(2, 2, 4L, 4L), row(3, 3, 4L, 4L)))
