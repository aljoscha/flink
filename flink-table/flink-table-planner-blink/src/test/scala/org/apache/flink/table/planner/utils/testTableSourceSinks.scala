...
object TestTableSourceSinks {
  def createPersonCsvTemporaryTable(tEnv: TableEnvironment, tableName: String): Unit = {
    tEnv
      .connect(new FileSystem().path(getPersonCsvPath))
      .withFormat(
        new OldCsv()
          .fieldDelimiter("#")
          .lineDelimiter("$")
  ...
  def createOrdersCsvTemporaryTable(tEnv: TableEnvironment, tableName: String): Unit = {
    tEnv
      .connect(new FileSystem().path(getOrdersCsvPath))
      .withFormat(
        new OldCsv()
          .fieldDelimiter(",")
          .lineDelimiter("$"))
  ...
  def createRatesCsvTemporaryTable(tEnv: TableEnvironment, tableName: String): Unit = {
    tEnv
      .connect(new FileSystem().path(getRatesCsvPath))
      .withFormat(
        new OldCsv()
          .fieldDelimiter(",")
          .lineDelimiter("$"))
  ...
  lazy val getOrdersCsvPath = {
    val csvRecords = Seq("2,Euro,2", "1,US Dollar,3", "50,Yen,4", "3,Euro,5", "5,US Dollar,6")

    writeToTempFile(csvRecords.mkString("$"), "csv-order-test", "tmp")
  }

  lazy val getRatesCsvPath = {
    val csvRecords = Seq("US Dollar,102", "Yen,1", "Euro,119", "RMB,702")
...
class TestPartitionableTableSource(
  ...
  override def getPartitions: JList[JMap[String, String]] = {
    ...
    List(
      ...
      Map("part1" -> "C", "part2" -> "1").asJava).asJava
  }
  ...
  override def getDataStream(execEnv: StreamExecutionEnvironment): DataStream[Row] = {
    val remainingData = if (remainingPartitions != null) {
      val remainingPartitionList = remainingPartitions.map { m =>
        s"part1=${m.get("part1")},part2=${m.get("part2")}"
      }
...
class WithoutTimeAttributesTableSource(bounded: Boolean) extends StreamTableSource[Row] {
  ...
  override def getDataStream(execEnv: StreamExecutionEnvironment): DataStream[Row] = {
    val data =
      Seq(Row.of("Mary", new JLong(1L), new JInt(1)), Row.of("Bob", new JLong(2L), new JInt(3)))
    val dataStream =
      execEnv
        .fromCollection(data)
