...
import org.apache.flink.table.functions.sql.{
  ProctimeSqlFunction,
  ScalarSqlFunctions,
...
abstract class CodeGenerator(
  ...
  def generateResultExpression(
      returnType: TypeInformation[_ <: Any],
      resultFieldNames: Seq[String],
      rexNodes: Seq[RexNode]): GeneratedExpression = {
  ...
  def generateResultExpression(
      ...
      returnType: TypeInformation[_ <: Any],
      resultFieldNames: Seq[String]): GeneratedExpression = {
    // initial type check
    ...
    returnType match {
      case ri: RowTypeInfo =>
        ...
        val resultSetters = boxedFieldExprs.zipWithIndex map { case (fieldExpr, i) =>
          if (nullCheck) {
            s"""
              |${fieldExpr.code}
              ...
              case ObjectPrivateFieldAccessor(field) =>
                ...
                if (nullCheck) {
                  s"""
                    |${fieldExpr.code}
                    |if (${fieldExpr.nullTerm}) {
                    |  ${reflectiveFieldWriteAccess(fieldTerm, field, outRecordTerm, defaultIfNull)};
                    |}
                    |else {
  ...
  override def visitCall(call: RexCall): GeneratedExpression = {
    if (call.getKind == SqlKind.SEARCH) {
      return RexUtil
        .expandSearch(new RexBuilder(FlinkTypeFactory.INSTANCE), null, call)
        .accept(this)
    }
    ...
    call.getOperator match {
      ...
      case sqlOperator: SqlOperator =>
        val callGen =
          FunctionGenerator.getCallGenerator(sqlOperator, operands.map(_.resultType), resultType)
        callGen
          .getOrElse(
            throw new CodeGenException(
              s"Unsupported call: $sqlOperator \n" +
  ...
  protected def makeReusableInSplits(expr: GeneratedExpression): GeneratedExpression = {
    ...
    if (expr.code.isEmpty) {
      ...
      // create method call
      GeneratedExpression(expr.resultTerm, expr.nullTerm, s"$methodName();", expr.resultType)
    }
  }
  ...
  private def generateCodeSplits(splits: Seq[String]): String = {
    ...
    if (totalLen > config.getMaxGeneratedCodeLength) {
      ...
      reusableInputUnboxingExprs.keys.foreach(key =>
        ...
        accessor match {
          case ObjectFieldAccessor(field) =>
            ...
            // Object
              ...
              generateInputFieldUnboxing(fieldType, s"($fieldTypeTerm) $inputTerm.${field.getName}")
            }
  ...
  private[flink] def generateNonNullLiteral(
      ...
      literalCode: String): GeneratedExpression = {
