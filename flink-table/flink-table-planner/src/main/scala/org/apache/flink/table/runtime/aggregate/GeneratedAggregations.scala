...
abstract class AggregationsFunction extends Function {
  ...
  /**
   * Setup method for [[org.apache.flink.table.functions.AggregateFunction]]. It can be used for
   * initialization work. By default, this method does nothing.
   *
   * @param ctx
   *   The runtime context.
   */
   ...
   *
   * @param accumulators
   *   the accumulators (saved in a row) which contains the current aggregated results
   * @param input
   *   input values bundled in a row
   */
   ...
   *
   * @param a
   *   First row of accumulators
   * @param b
   *   The other row of accumulators
   * @return
   *   A row with the merged accumulators of both input rows.
   */
   ...
   *   input values bundled in a row
   * @param output
   *   output results collected in a row
   */
   ...
   *
   * @return
   *   an output row object with the correct arity.
   */
  ...
  /**
   * Tear-down method for [[org.apache.flink.table.functions.AggregateFunction]]. It can be used for
   * clean up work. By default, this method does nothing.
   */
  def close()
}
...
abstract class GeneratedAggregations extends AggregationsFunction {
  ...
  /**
   * Sets the results of the aggregations (partial or final) to the output row. Final results are
   * computed with the aggregation function. Partial results are the accumulators themselves.
   *
   * @param accumulators
   *   the accumulators (saved in a row) which contains the current aggregated results
   * @param output
