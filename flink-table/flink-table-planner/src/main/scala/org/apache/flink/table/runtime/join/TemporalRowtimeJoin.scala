...
class TemporalRowtimeJoin(
   ...
   *
   * TODO: this could be OrderedMultiMap[Jlong, Row] indexed by row's timestamp, to avoid full map
   * traversals (if we have lots of rows on the state that exceed `currentWatermark`).
   */
   ...
   *
   * TODO: having `rightState` as an OrderedMapState would allow us to avoid sorting cost once per
   * watermark
   */
  ...
  /**
   * @return
   *   a row time of the oldest unprocessed probe record or Long.MaxValue, if all records have been
   *   processed.
   */
  ...
  private def indexOfFirstElementNewerThanTimer(timerTimestamp: Long, list: util.List[Row]): Int = {
  ...
  /**
   * Binary search `rightRowsSorted` to find the latest right row to join with `leftTime`. Latest
   * means a right row with largest time that is still smaller or equal to `leftTime`.
   *
   * @return
   *   found element or `Optional.empty` If such row was not found (either `rightRowsSorted` is
   *   empty or all `rightRowsSorted` are are newer).
   */
