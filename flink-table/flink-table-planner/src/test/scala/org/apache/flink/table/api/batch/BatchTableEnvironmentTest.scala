...
class BatchTableEnvironmentTest extends TableTestBase {
  ...
  def testSqlWithoutRegistering(): Unit = {
    ...
    val join = unaryNode(
      "DataSetJoin",
      binaryNode("DataSetCalc", batchTableNode(table), batchTableNode(table2), term("select", "c")),
      term("where", "=(c, d)"),
      term("join", "c, d, e, f"),
      term("joinType", "InnerJoin"))

    val expected2 = unaryNode("DataSetCalc", join, term("select", "d, e, f"))
  ...
  def testExecuteSqlWithCreateDropTable(): Unit = {
    ...
    assertTrue(
      util.tableEnv
        .getCatalog(util.tableEnv.getCurrentCatalog)
        .get()
    ...
    assertEquals(
      ...
      util.tableEnv
        .getCatalog(util.tableEnv.getCurrentCatalog)
        .get()
        .getTable(ObjectPath.fromString(s"${util.tableEnv.getCurrentDatabase}.tbl1"))
        .getProperties)
  ...
  def testExecuteSqlWithCreateDropFunction(): Unit = {
    ...
    val tableResult1 =
      util.tableEnv.executeSql(s"CREATE FUNCTION default_database.f1 AS '$funcName'")
    assertEquals(ResultKind.SUCCESS, tableResult1.getResultKind)
    assertTrue(
      util.tableEnv
        .getCatalog(util.tableEnv.getCurrentCatalog)
    ...
    val tableResult2 =
      util.tableEnv.executeSql(s"ALTER FUNCTION default_database.f1 AS '$funcName2'")
    assertEquals(ResultKind.SUCCESS, tableResult2.getResultKind)
    assertTrue(
      util.tableEnv
        .getCatalog(util.tableEnv.getCurrentCatalog)
    ...
    val tableResult4 =
      util.tableEnv.executeSql(s"CREATE TEMPORARY SYSTEM FUNCTION f2 AS '$funcName'")
    assertEquals(ResultKind.SUCCESS, tableResult4.getResultKind)
    assertTrue(util.tableEnv.listUserDefinedFunctions().contains("f2"))

    val tableResult5 = util.tableEnv.executeSql("DROP TEMPORARY SYSTEM FUNCTION f2")
