...
class CorrelateTest extends TableTestBase {
  ...
  def testCrossJoin(): Unit = {
    ...
    val expected1 = unaryNode(
      ...
      unaryNode(
        ...
        term("select", "a", "b", "c", "s"),
        term("rowType", "RecordType(INTEGER a, BIGINT b, VARCHAR(65536) c, VARCHAR(65536) s)"),
        term("joinType", "INNER")),
      term("select", "c", "s"))
  ...
  def testNonCompositeResultType(): Unit = {
    ...
    val expected = unaryNode(
      ...
      term("rowType", "RecordType(INTEGER f0, BIGINT f1, VARCHAR(65536) f2, VARCHAR(65536) f0_0)"),
      term("joinType", "INNER"))
  ...
  def testCorrelatePythonTableFunction(): Unit = {
    ...
    val expected = unaryNode(
      ...
      term("rowType", "RecordType(INTEGER a, INTEGER b, INTEGER c, INTEGER x, INTEGER y)"),
      term("joinType", "INNER"))
    util.verifyTable(resultTable, expected)
  }
}
