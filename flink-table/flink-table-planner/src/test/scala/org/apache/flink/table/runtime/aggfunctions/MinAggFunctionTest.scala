...
class StringMinAggFunctionTest extends AggFunctionTestBase[String, MinAccumulator[String]] {
  override def inputValueSets: Seq[Seq[_]] = Seq(
    Seq(
      new String("a"),
      new String("b"),
  ...
  override def expectedResults: Seq[String] =
    Seq(new String("a"), null.asInstanceOf[String], new String("1House"))
...
class TimestampMinAggFunctionTest
  ...
  override def expectedResults: Seq[Timestamp] = Seq(new Timestamp(0), null.asInstanceOf[Timestamp])

  override def aggregator: AggregateFunction[Timestamp, MinAccumulator[Timestamp]] =
    new TimestampMinAggFunction()
}

class DateMinAggFunctionTest extends AggFunctionTestBase[Date, MinAccumulator[Date]] {
  override def inputValueSets: Seq[Seq[_]] = Seq(
  ...
  override def expectedResults: Seq[Date] = Seq(new Date(0), null.asInstanceOf[Date])

  override def aggregator: AggregateFunction[Date, MinAccumulator[Date]] =
    new DateMinAggFunction()
}

class TimeMinAggFunctionTest extends AggFunctionTestBase[Time, MinAccumulator[Time]] {
  override def inputValueSets: Seq[Seq[_]] = Seq(
  ...
  override def expectedResults: Seq[Time] = Seq(new Time(0), null.asInstanceOf[Time])
