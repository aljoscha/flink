...
import org.apache.flink.api.java.operators.translation.{
  KeyExtractingMapper,
...
class ReduceTranslationTest {
  ...
  def translateNonGroupedReduce(): Unit = {
    try {
      ...
      initialData reduce { (v1, v2) => v1 } output (new DiscardingOutputFormat[(
          Double,
          String,
          Long)])
  ...
  def translateGroupedReduceNoMapper(): Unit = {
    try {
      ...
      initialData.groupBy(2) reduce { (v1, v2) => v1 } output (new DiscardingOutputFormat[(
          Double,
          String,
          Long)])
  ...
  def translateGroupedReduceWithKeyExtractor(): Unit = {
    try {
      ...
      initialData.groupBy { _._2 }.reduce { (v1, v2) =>
        v1
      } setParallelism (4) output (new DiscardingOutputFormat[(Double, String, Long)])

      val p = env.createProgramPlan()
      val sink: GenericDataSinkBase[_] = p.getDataSinks.iterator.next
      val keyProjector: MapOperatorBase[_, _, _] =
